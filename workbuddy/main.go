	...
	return registration{
		...
		Metadata: pluginapi.Metadata{
			...
			ConfigFields: []pluginapi.ConfigField{
				...
				{Name: "scheduler_mode", Type: pluginapi.ConfigFieldTypeEnum, EnumValues: []string{schedulerModeOff, schedulerModeCredits, schedulerModeSession}, Description: "Multi-account selection: off (defer to built-in, default), credits (panel-selected sticky account), or session (per-conversation round-robin, same conversation keeps one account for 1h). WARNING: when off + lifecycle_auto=false, exhausted accounts may still be routed — enable lifecycle_auto or set scheduler_mode=credits/session."},
